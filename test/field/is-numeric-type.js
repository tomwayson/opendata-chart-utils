...
QUnit.test('it returns true for numeric types', function (assert) {
  ...
  types.forEach(function (type) {
    assert.ok(opendataChartUtils.field.isNumericType(type), type + ' should be numeric');
  });
});
...
QUnit.test('it returns fals for non-numeric types', function (assert) {
  ...
  types.forEach(function (type) {
    assert.notOk(opendataChartUtils.field.isNumericType(type), type + ' should not be numeric');
  });
});
